...
export const SocialActions: React.FC<SocialActionsProps> = ({
  ...
  // State for like, save, and share dialog
  ...
  return (
    <>
      <div className={`flex ${horizontal ? 'flex-row' : 'flex-col'} items-center gap-2 ${className}`}>
        {/* Like Button */}
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <div className="flex items-center gap-1">
                  ...
                  {showLabels && <span className="text-sm font-medium">{isLiked ? 'Liked' : 'Like'}</span>}
                </div>
              </motion.button>
            </TooltipTrigger>
            <TooltipContent>
            ...
            <TooltipTrigger asChild>
              ...
              >
                <div className="flex items-center gap-1">
                  ...
                  {showLabels && <span className="text-sm font-medium">{isSaved ? 'Saved' : 'Save'}</span>}
                </div>
              </motion.button>
            </TooltipTrigger>
            <TooltipContent>
              <p>{isSaved ? 'Unsave' : 'Save for later'}</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <motion.button
                onClick={() => handleShare()}
                className={`${getSizeClasses()} bg-white text-gray-600 border border-gray-200 hover:bg-gray-50 transition-all`}
                whileHover={{
                  scale: 1.1,
                  y: -2,
                  boxShadow: "0 5px 10px rgba(0, 0, 0, 0.05)"
      ...
      <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-gradient-to-br from-white to-gray-50 border-0">
          ...
          >
            ...
            <DialogTitle className="text-2xl font-bold text-white mb-2">Share this {contentType}</DialogTitle>
            ...
            <DialogClose className="absolute top-4 right-4 rounded-full p-1.5 text-white/80 opacity-70 hover:bg-white/10 hover:opacity-100 focus:outline-none">
              <X className="w-5 h-5" />
